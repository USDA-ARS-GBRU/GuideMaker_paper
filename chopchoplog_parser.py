...
def myparser():
    ...
    parser = argparse.ArgumentParser(
        description="chopchoplog_parser.py: A script to reformat slurm log files.")
    parser.add_argument('--indir', '-i', type=str, required=True,
                        help='The error log produced from chopchop local run')
...
def main(args=None):
    ...
    datalist = []
    for file in glob.glob(os.path.join(os.path.join(args.indir, "*.log"))):
        try:
    ...
    chunks = [datalist_flat[x:x+3] for x in range(0, len(datalist_flat), 3)]
    labels = ["threads", "process_sec", "MemoryUsage"]
